...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function doStartUp() {
    function pushText(txt, spd) {
      return new Promise((resolve) => {
        let index = 0;
        const typingSpeed = Math.max(Math.floor(spd / txt.length), 20); // Ensure typing speed isn't too fast
        const typeLetter = () => {
          if (index < txt.length) {
    ...
    for (const [line, delay] of bootup) {
      await new Promise(resolve => setTimeout(resolve, delay - 20));
      await pushText(line, delay);
    }
